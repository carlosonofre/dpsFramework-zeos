        ...
        begin
          try
            case rsmd.GetColumnType(i) of
              ...
              stBinaryStream:
                begin
                  if rsetOrigin.GetBinaryStream(i).Size > 0 then
                    SetPropValue(self ,rsmd.GetColumnName(i), rsetOrigin.GetBinaryStream(i).ToString);
                     //  if Length( TBlobField(qrOrigin.Fields[i]).Value) > 0 then
...
begin
  ...
  try
    ...
    for i := 1 to rsmd.GetColumnCount do
    begin
      if SameText(rsmd.GetColumnTypeName(i),'Blob') then
      begin
        if length( GetPropValue(Self,rsmd.GetColumnName(i))) > 0  then
          rset.GetBlob(i).SetString(GetPropValue(Self,rsmd.GetColumnName(i)))
