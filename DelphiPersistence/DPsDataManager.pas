...
uses
 ...
 Data.DBXJSON,Data.DBXJSONCommon,
...
type
 ...
 TDpsAbstractModel = class;
 IDPSAbstractModel = class end;
 TClassAbstractModel = class of TDpsAbstractModel;
 ...
 IDataManager = interface ['{0A41F59B-7B1A-4B09-8D87-252ADDCF3689}']
    ...
    function get(sql:string):IZResultSet;                                       (* implementado *)
    function exec(sql:string):boolean;                                          (* implementado *)
 ...
 public
    ...
    (* principal método de retorno de dados *)
    function get(sql:string):IZResultSet;                                       (* implementado *)
    function exec(sql:string):Boolean;                                          (* implementado *)
 ...
 IDataModel = Interface ['{84E1298E-2498-4038-B608-A4C785393D23}']
    ...
    function getPropertyReflection(propName:String):TDataReflection;
    ...
    procedure setKeyFieldAutoGenerated(const Value:Boolean);
    procedure fillProperties(rsetOrigin:IZResultSet);
    procedure fillDataset(qrDestination:TClientDataSet);
    ...
    procedure SetupKeyField(keyField:String);
    function LoadFileAsBlob(sFileName:String):TBlobData;
    function LoadMemoryStreamAsBlob(memoryStream:TMemoryStream):TBlobData;

 (*  C.R.U.D *)
    function save:Boolean;
    function delete:Boolean;
    //    function get(sSelect,sWhere,sOrder:String;params:TStrings; outDataSet:TClientDataSet = nil):IZResultSet;
  ...
  TDpsAbstractModel = class(TInterfacedObject,IDataModel)
  private
  ...
  public
   ...
   (* Interfaces *)
    ...
    procedure UpdateLastID(ZeroToEmptyTable:Boolean=False);
    function ValidarRegras:Boolean;virtual;
    function LoadFileAsBlob(sFileName:String):TBlobData;
    function LoadMemoryStreamAsBlob(memoryStream:TMemoryStream):TBlobData;
   ...
   (*c r u d*)
    ...
    function delete:Boolean;
    function getSQL(sql:String):IZResultSet;
...
//   function getList(sWhere:String = '';sOrder:String = '';externalDataSet:TClientDataSet = nil):TObjectList;
  ...
  (* Padrão CODS , tem duas proriedades básicas *)
  TDpsStandardModel = class(TDPSAbstractModel)
  private
    FId   :Int64;
    FGuid :STR_GUID;
  public
    procedure Token;virtual;
    constructor Create(dmgr:IDataManager);override;
    function ValidarRegras:Boolean;reintroduce;override;
  published
  ...
  const
    ...
    (*  nomes de parâmetros lidos do arquivo .ini   *)
...
resourcestring
  ...
  SErrorTokenizerNotDefinedInModel        = 'A Classe: [ %s ] não possui implementação de Token. ';
...
const
  ...
  codSErrorTokenizerNotDefinedInModel     = '000023';
  ...
  with TIniFile.Create(getFullPathIniFileConfig) do
    begin
      try
        ...
        if  not (SectionExists(pSessionName))
        or  not (ValueExists(pSessionName,pName)) then
  ...
  try
    ...
    begin
      ...
      begin
        ...
        Add('{                       Se você usa um SVN ou outro SCM com o tempo terá um bom}');
...
begin
  Result := TZConnection.Create(application);
  try
...
function TAbstractDataManager.get(sql: string): IZResultSet;
var
...
begin
  ...
  try
    ...
    Result := st.ExecuteQueryPrepared;
  except
    on e:Exception do
...
function TDPSAbstractModel.getPropertyReflection(propName: String): TDataReflection;
var
 i:integer;
begin
  for i:=0 to length(FModelReflection)-1 do
  begin
    if SameText(propName,FModelReflection[i].PropertyOrColumnName) then
      Result := FModelReflection[i];
  end;
end;

function TDpsAbstractModel.getSQL(sql: String): IZResultSet;
begin
  Result := Datamanager.getConnection.PrepareStatement(sql).ExecuteQueryPrepared;
end;
...
function TDpsAbstractModel.LoadFileAsBlob(sFileName:String): TBlobData;
var
  ms : TMemoryStream;
begin
  SetLength(Result, 0);
...
function TDpsAbstractModel.LoadMemoryStreamAsBlob(memoryStream: TMemoryStream): TBlobData;
begin
  SetLength(Result, 0);
  ...
  begin
    ...
    memoryStream.Read(Result[0], memoryStream.Size);
  end;
  Result := @Result[0];
end;
...
begin
  ...
  rsetDel := Datamanager.get(sql);
  try
    if rsetDel.next then
      rsetDel.DeleteRow
  except
...
procedure TDPSAbstractModel.fillproperties(rsetOrigin:IZResultSet);
var
  i:Integer;
  rsmd : IZResultSetMetadata;
begin
    FDatamanager.ResetLogBusinessRule;
    rsmd := rsetOrigin.GetMetadata;
    if rsetOrigin.Next then
    begin
      try
        for i:= 1 to rsmd.GetColumnCount do
        begin
          try
            case rsmd.GetColumnType(i) of
              stDate,
              stTime,
              stTimestamp:begin
                            SetPropValue(self,rsmd.GetColumnName(i),MinDateTime);
                            if rsetOrigin.GetDate(i) > MinDateTime then
                              SetPropValue(self,rsmd.GetColumnName(i),rsetOrigin.GetDate(i));
                          end;
              stFloat:   SetPropValue(self,rsmd.GetColumnName(i), rsetOrigin.GetFloat(i));
              stInteger: SetPropValue(self,rsmd.GetColumnName(i), rsetOrigin.GetLong(i));
              stDouble:  SetPropValue(self,rsmd.GetColumnName(i), rsetOrigin.GetDouble(i));
              stShort:   SetPropValue(self,rsmd.GetColumnName(i), rsetOrigin.GetShort(i));
              stBinaryStream:
                begin
                    SetPropValue(self ,rsmd.GetColumnName(i),rsetOrigin.GetBlob(i).GetBytes);
//                    SetPropValue(self ,rsmd.GetColumnName(i),rsetOrigin.GetBlob(i));  referencia
                end;
              else
                SetPropValue(self,rsmd.GetColumnName(i), rsetOrigin.GetString(i));
              end;
          except
            on e:exception do
            begin
              Datamanager.AddLogBusinessRule(codSFillPropertiesErrorInProperty,
                                             format(SFillPropertiesErrorInProperty,[rsmd.GetColumnName(i),rsetOrigin.GetString(i)])
                                             +#13#10+'*** '+e.Message,
                                             sbrWarning);
              continue;
            end;
...
procedure TDpsAbstractModel.SetupKeyField(keyField: String);
begin
...
procedure TDPSAbstractModel.UpdateLastID(ZeroToEmptyTable:Boolean);
var
  sql:string;
  rset:IZResultSet;
  lastid:Int64;
begin
  ...
  case Datamanager.TypeDatabase of
    tdbFirebird:
      begin
        sql  := format('select max(%s)lastid from %s',[KeyFieldName,TableName]);
        rset := Datamanager.get(sql);
        if rset.next then
          lastid := rset.GetIntByName('lastid');
        if (lastid > -1 )then
        begin
          if (ZeroToEmptyTable) or (lastid > 0) then
            Datamanager.ExecuteScript(format('SET GENERATOR %s TO %d',[UpperCase(TableName),lastid]));
...
function TDpsAbstractModel.ValidarRegras: Boolean;
begin
  Result:=False;
end;

procedure TDPSAbstractModel.fillDataset(qrDestination: TClientDataSet);
var
  ifield:integer;
begin
  FDatamanager.ResetLogBusinessRule;
  ...
  begin
    self.configureExternalDataset(qrDestination);
    try
...
//begin
//  Result := nil;
//  if (trim(ident) = '') or  (StrToInt64Def(trim(ident),-1) < 0) then
//    sql := format('select * from %s where %s = -1',[TableName,KeyFieldName])
//  else
//    sql := format('select * from %s where %s = %s',[TableName,KeyFieldName,ident]);
//
//  if (Trim(KeyFieldName) <> '') then
//  begin
//    st   := Datamanager.getConnection.PrepareStatement(sql);
//    rset := st.ExecuteQueryPrepared;
//
//    if not (rset.next) or (externalDataSet <> nil) then
//      Result := New(Datamanager,rset);
//
//    if Result <> nil then
//      Result.fillDataset(externalDataSet);
//  end;
...
begin
  ...
  Result := false;
  Insert := false;
  sql    := format('select * from %s where (%s = %s) ',[self.TableName,self.KeyFieldName,GetPropValue(self,Self.KeyFieldName)]);
  st     := Datamanager.getConnection.PrepareStatement(sql);
  rset   := st.ExecuteQueryPrepared;
  (* set writable *)
  TZAbstractResultSet(rset).SetConcurrency(rcUpdatable);
  rsmd := rset.GetMetadata;
  try
    insert := not rset.next();
    if insert then
    begin
      rset.MoveToInsertRow;
      if KeyFieldAutoGenerated then
        SetPropValue(self,KeyFieldName,IntToStr(GenerateNewID));
    end;

    if InheritsFrom(TDPSStandardModel) then
      TDPSStandardModel(Self).Token;

    for i := 1 to rsmd.GetColumnCount do
    begin
      if (rsmd.GetColumnType(i) <> stBinaryStream ) then
        rset.UpdateString(i,GetPropValue(Self,rsmd.GetColumnName(i)))
      else
        rset.UpdateBytes(i,VarToBytes(GetPropValue(Self,rsmd.GetColumnName(i))));
    end;
...
begin
  criar := false;
  criar := (   SameText( getParam(dpsHOST),'127.0.0.1')
            or SameText( getParam(dpsHOST),'localhost')
            or SameText( getParam(dpsHOST),''))
            and (getParam(dpsPATH)<>'')
            and not (FileExists(getParam(dpsPATH)));
  if criar then
  begin
    p := getParam(dpsPATH);
    while ( p[length(p)]<>'\') and (length(p)>1) do
      delete(p,length(p),1);
    if trim(p) <> '' then
      ForceDirectories(p);
  end;
        ...
        for j:=0 to length(FTableReflections[i])-1 do
          begin
            if (FTableReflections[i][j].AutoInc) and (FTableReflections[i][j].PrimaryKey) then
              if not GeneratorExists(format('%s',[FTableReflections[i][j].ClassOrTableName]))
                 then ExecuteScript( format('create generator %s',[FTableReflections[i][j].ClassOrTableName]));
          end;
      end;
end;
...
function TAbstractDataManager.exec(sql: string): Boolean;
begin
  Result := false;
  try
    Result := getConnection.PrepareStatement(sql).ExecutePrepared;
  except
    on e:Exception do
      raise EDPSException.Create(e.Message);
  end;
end;
...
begin
  ...
  case Datamanager.TypeDatabase of
    ...
    begin
      UpdateLastID;
      sql_query := format('select gen_id(%s,%d)new_id from rdb$database',[UpperCase(TableName),inc]);
      rSetID    := Datamanager.get(sql_query);
      if rSetID.Next then
        Result := rSetID.GetIntByName('new_id');
    end;
  end;
end;


procedure TDpsStandardModel.Token;
begin
  if trim(Tokenizer) = '' then
    Datamanager.AddLogBusinessRule(codSErrorTokenizerNotDefinedInModel,Format(SErrorTokenizerNotDefinedInModel,[self.ClassName]),sbrError)
  else
    FGuid := Trim(GeraMD5(Tokenizer));
end;

function TDpsStandardModel.ValidarRegras: Boolean;
begin
  Result := false;
  Token;
  Datamanager.ResetLogBusinessRule;
end;
