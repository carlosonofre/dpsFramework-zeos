...
begin
  Result := TZQuery.Create(Application);
  ...
  case Datamanager.TypeDatabase of
    ...
    begin
      try
        ...
        if lastid > 0
          then Datamanager.ExecuteScript(format('SET GENERATOR %s TO %d',[UpperCase(TableName),lastid]));
      finally
...
begin
  ...
  try
    ...
    if getPropertyReflection(KeyFieldName).AutoInc then
